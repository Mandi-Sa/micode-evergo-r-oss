...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x001e, 0x0004);
	write_cmos_sensor(0x6028, 0x4000);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
    LOG_INF("get_imgsensor_idzzz sensor_id: 0x%x\n",*sensor_id);
    ...
    LOG_INF("get_imgsensor_idzzzz sensor_id: 0x%x\n",*sensor_id);
if(*sensor_id == 0x38e1)
*sensor_id = imgsensor_info.sensor_id;
    LOG_INF("get_imgsensor_idzzzzz sensor_id: 0x%x\n",*sensor_id);
				if (*sensor_id == imgsensor_info.sensor_id) {
					pr_info
			("s5kjn1sunny_ofilm i2c 0x%x, sid 0x%x\n",
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			LOG_INF("open zzz sensor_id: 0x%x, imgsensor_info.sensor_id \n", sensor_id);
if(sensor_id == 0x38e1)
sensor_id = imgsensor_info.sensor_id;
			LOG_INF
		("open zzzjy , Rsid: 0x%x, info.sid:0x%x.\n",
			  sensor_id,
			 imgsensor_info.sensor_id);
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF
				("i2c write id zzzjy: 0x%x, sensor id: 0x%x\n",
				 imgsensor.i2c_write_id, sensor_id);
				break;
			}
			LOG_INF
			("Read sensor id fail zzzjy, id: 0x%x, sensor id: 0x%x\n",
			 imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
