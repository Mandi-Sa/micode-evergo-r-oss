...
static int check_if_bug_on(void)
{
	char *reboot_bootloader = "bootloader";

	if (ufshcd_get_hwlevel()) {
		pr_info("%s: P2.1/MP phone is not enable write booster feature, factory image can not turn on!\n", __func__);
...
int check_wb_hpb_size(struct ufs_hba *hba)
{
	...
	pr_info("ufs total:%dGB wb:%dGB hpb:%dGB\n",
		hba->check.total_gb, hba->check.wb_gb, hba->check.hpb_gb);
	...
	if (hba->check.total_gb > 512 && hba->check.total_gb <= 1024)
		...
		pr_err("current density is too low.\n");
		return -1;
